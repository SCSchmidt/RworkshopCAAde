---
title: "Code-Skript R Workshop @ CAA de 2019"
...
### Option: Schleife schreiben (wenn noch Zeit ist)

Mit Schleifen lassen sich Prozesse automatisieren und für mehrere Datensätze gleichzeitig durchführen. Es gibt "for", "while" und "repeat"-Schleifen. Als Beispiel betrachten wir hier nur die "for"-Schleife.

Die Aufgabe für R ist immer "für jedes Element i in einem Vektor v tu das und das":

for(i in v) {
tu das hier
}

Nehmen wir als Beispiel den BACups-Datensatz und addieren jeden Wert mit 5 (meine Kreativität kennt keine Grenzen...):
...
```
Der Print-Befehl ist wichtig, damit uns nicht nur das letzte Ergebnis angezeigt wird, sondern alle. Für die letzte Spalte bekommen wir eine Warnung: "Argument ist weder numerisch noch boolesch: gebe NA zurück[1] NA", weil dies die "Phase"-Spalte ist, in der keine numerischen Werte vorkommen und folglich kein Mittelwert berechnet werden kann.

Nur zur Info: Die Aufgabe kann noch schneller gelöst werden:

```{r}
colMeans(BACups[sapply(BACups, is.numeric)])
```
...
5. googeln: Online gibt es viele Leute, die das gleiche Problem schon hatten. Beim googeln hilft es, den Paket- und den Funktionsnamen "mitzugoogeln". Da "R" als Buchstabe zu unspezifisch ist, gibt es außerdem https://rseek.org/ , eine Art "personalisiertes Google".
